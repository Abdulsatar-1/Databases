...
CREATE TABLE `restaurant_rating` (
  ...
  `ambience` float DEFAULT NULL,
  `food_quality` float DEFAULT NULL,
  `service` float DEFAULT NULL,
  `price` float DEFAULT NULL,
  `overall_experience` float DEFAULT NULL,
  PRIMARY KEY (`restaurant_rating_id`),
  UNIQUE KEY `restaurant_review_unique` (`restaurant_id`),
  CONSTRAINT `restaurant_rating_ibfk_1` FOREIGN KEY (`restaurant_id`) REFERENCES `restaurant` (`restaurant_id`) ON DELETE CASCADE
